...
RSpec.describe Rules::PrimeIdSanctionRule do
  ...
  it 'passes for non-prime id regardless of other primes' do
    add_load(id: '2')
    cand = builder.build('id' => '8', 'customer_id' => '10', 'load_amount' => '$99999.00', 'time' => date,
                         'accepted' => true)
  ...
  it 'fails for prime id with amount > 9999' do
    cand = builder.build('id' => '3', 'customer_id' => '10', 'load_amount' => '$10000.00', 'time' => date,
                         'accepted' => true)
  ...
  it 'allows only one prime id per date globally' do
    ...
    cand = builder.build('id' => '5', 'customer_id' => '10', 'load_amount' => '$1.00', 'time' => date,
                         'accepted' => true)
    expect(rule.acceptable?(cand)).to be false
  end

  it 'allows a single prime id on that date' do
    cand = builder.build('id' => '5', 'customer_id' => '10', 'load_amount' => '$9.00', 'time' => date,
                         'accepted' => true)
    expect(rule.acceptable?(cand)).to be true
  end
end
