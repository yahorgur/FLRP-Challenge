...
module Rules
  # Max $20000 per customer per ISO week (effective_amount)
  class WeeklyAmountLimitRule < Base
    ...
    def acceptable?(candidate)
      ...
      week = fund_load_repository.accepted.find_by_customer_id(candidate.customer_id).records
                                 .select { |r| Utils::DateUtils.same_iso_week?(r.timestamp, candidate.timestamp) }
      sum = week.sum(&:effective_amount)
      (sum + candidate.effective_amount) <= LIMIT
    end
  end
end
